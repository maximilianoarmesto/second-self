 ...
 *    a `model` string, an array of `messages`, and a provider-specific `apiKey`.
 ...
 *
 * @throws Will re-throw any SDK errors so the caller can handle them
 *         (e.g. invalid key, rate-limit, network failure).
 */
...
async function callAnthropic(
  ...
  // this shape (system → user/assistant history → user question), but if the
  // remaining messages are empty we add a minimal user prompt to prevent an
