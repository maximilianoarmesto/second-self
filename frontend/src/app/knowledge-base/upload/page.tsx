...
export default function UploadPage() {
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    for (const file of fileArray) {
      ...
      if (file.size > MAX_FILE_SIZE_BYTES) {
        // Add as error entry so the user can see the size issue
        valid.push({
  ...
  const uploadFile = async (uploadFile: UploadFile) => {
    setFiles((prev) =>
      prev.map((f) => (f.id === uploadFile.id ? { ...f, state: 'uploading', progress: 0 } : f))
    );

    try {
      const response = await documentsApi.upload(uploadFile.file, (pct) => {
        setFiles((prev) =>
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === uploadFile.id
            ? { ...f, state: 'success', progress: 100, result: response.data }
            : f
        )
      );
    } catch (err: unknown) {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === uploadFile.id ? { ...f, state: 'error', errorMessage: message } : f
        )
      );
    }
  };
